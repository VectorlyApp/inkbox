...
let count = 0;
for await (const msg of inkbox.messages.list(mailboxAddress)) {
...
for await (const thread of inkbox.threads.list(mailboxAddress)) {
  console.log(`${thread.id}  "${thread.subject}"  messages=${thread.messageCount}`);
  firstThreadId ??= thread.id;
}

if (firstThreadId) {
  const thread = await inkbox.threads.get(mailboxAddress, firstThreadId);
